...
from redisvl.redis.utils import buffer_to_array, convert_bytes
...
class RedisVectorStore(VectorStore):
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        out_keys = vector_store.add_texts(texts, metadatas, keys)

        if return_keys:
            return vector_store, out_keys
        else:
            return vector_store
    ...
    def delete(self, keys: List[str]):
        ...
        return self._index.delete(keys)
    ...
    def similarity_search_with_score_by_vector(
        ...
        filter: Optional[FilterExpression] = None,
        **kwargs: Any,
    ) -> List[Tuple[Document, float, Optional[np.ndarray]]]:
        """Return docs most similar to embedding vector.
        ...
        if not return_all:
            if with_vectors:
                ...
                docs_from_storage = self._index._storage.get(self._index.client, doc_ids)
            ...
            docs_with_scores = [
                (
                    ...
                    buffer_to_array(doc.get(self.config.embedding_field)) if with_vectors else None,
                )
    ...
    def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        documents = [doc for doc, _, _ in docs_scores_embeddings]
        embeddings = [embedding for _, _, embedding in docs_scores_embeddings]
