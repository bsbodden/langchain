...
def test_redis_from_existing(texts: List[str], redis_url: str) -> None:
    ...
    schema: Dict = vector_store.index.schema
...
def test_redis_from_texts_return_keys(redis_url: str, texts: List[str]) -> None:
    ...
    # Create a unique index name for testing
    index_name = f"test_index_{str(ULID())}"
    vector_store, keys = RedisVectorStore.from_texts(
        texts,
        OpenAIEmbeddings(),
        index_name=index_name,
        key_prefix="tst4",
...
def test_custom_keys(texts: List[str], redis_url: str) -> None:
    ...
    index_name = f"test_index_{str(ULID())}"
    vector_store, keys_out = RedisVectorStore.from_texts(
        texts,
        OpenAIEmbeddings(),
        index_name=index_name,
        key_prefix="tst7",
    ...
    assert keys_out == [vector_store.key_prefix + ":" + key for key in keys_in]
...
def test_custom_keys_from_docs(texts: List[str], redis_url: str) -> None:
    ...
    index_name = f"test_index_{str(ULID())}"
    vector_store, keys_out = RedisVectorStore.from_documents(
        docs,
        OpenAIEmbeddings(),
        index_name=index_name,
        key_prefix="tst8",
    ...
    assert client.hget(vector_store.key_prefix + ":" + "test_key_1", "text")
    # # test metadata is stored
    assert client.hget(vector_store.key_prefix + ":" + "test_key_1", "a") == bytes(
        "b", "utf-8"
    )
    # test all keys are stored
    assert client.hget(vector_store.key_prefix + ":" + "test_key_2", "text")
    # Clean up
    vector_store.index.delete(drop=True)
...
def test_similarity_search_with_metadata_filtering(redis_url: str) -> None:
    ...
    metadatas = [
        ...
        },
        {
            ...
            "review": "The Mustang is a thrilling car to drive with its powerful engine.",
        },
    ]
...
def test_similarity_search_with_score_with_limit_distance(redis_url: str) -> None:
    ...
    for doc, score, vector in output:
...
def test_similarity_search_with_score_by_vector_with_extra_fields(
    ...
    assert len(results_without_return_all) == 2
    for doc, score, _ in results_without_return_all:
        assert doc.page_content in texts
        assert "indexed_metadata" in doc.metadata
        assert doc.metadata["indexed_metadata"].startswith("doc")
        assert "extra_field1" not in doc.metadata
    ...
    assert len(results_with_return_all) == 2
    for doc, score, _ in results_with_return_all:
        assert doc.page_content in texts
        assert "indexed_metadata" in doc.metadata
        assert doc.metadata["indexed_metadata"].startswith("doc")
        assert "extra_field1" in doc.metadata
    ...
    for doc, score, vector in results_with_vectors:
        assert doc.page_content in texts
        assert "indexed_metadata" in doc.metadata
        assert "extra_field1" in doc.metadata
        assert "extra_field2" in doc.metadata
...
def test_connect_to_redisvl_created_index_w_index_name(redis_url: str) -> None:
    ...
    schema = IndexSchema.from_dict({
        ...
        "fields": [
            {"name": "text", "type": "text"},
            {"name": "embedding", "type": "vector", "attrs": {"dims": 3072, "distance_metric": "cosine", "algorithm": "FLAT"}},
        ]
    })
    ...
    keys = redisvl_index.load(data, keys=[f"{index_name}:{label}" for label in sentences.keys()])
    print(f"Added {len(sentences)} documents with keys: {keys}")
    ...
    for label, text in sentences.items():
        ...
        print(f"Document '{label}' exists in Redis")
        print(f"Document '{label}' content: {redis_client.hgetall(key)}")
    ...
    print(f"RedisVL Vector search results: {redisvl_results}")

    # LangChain search
    langchain_results = langchain_vector_store.similarity_search(query_text, k=3)
    print(f"LangChain search results: {[doc.page_content for doc in langchain_results]}")

    # Assertions
    assert len(langchain_results) == 3, f"Expected 3 results, got {len(langchain_results)}"
    assert langchain_results[0].page_content == "I like dogs.", "Expected 'I like dogs.' to be the top result"
    assert langchain_results[1].page_content == "You love dogs.", "Expected 'You love dogs.' to be the second result"
    assert langchain_results[2].page_content == "I hate dogs.", "Expected 'I hate dogs.' to be the third result"
