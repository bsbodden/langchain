...
def test_clear_messages(chat_history):
...
def test_add_multiple_messages(chat_history):
    messages = [
    ...
    assert [msg.content for msg in chat_history.messages] == ["Message 1", "Response 1", "Message 2"]

def test_search_messages(chat_history):
...
def test_ttl(redis_url, redis_client):
    session_id = f"ttl_test_{str(ULID())}"
    chat_history = RedisChatMessageHistory(session_id=session_id, redis_url=redis_url, ttl=1)
    ...
    assert redis_client.ttl(message_key) > 0
...
def test_multiple_sessions(redis_url):
...
def test_index_creation(redis_client, redis_url):
...
def test_different_message_types(chat_history, message_type):
...
def test_large_number_of_messages(chat_history):
    large_number = 1000
    messages = [HumanMessage(content=f"Message {i}") for i in range(large_number)]
    for message in messages:
        chat_history.add_message(message)
    ...
    for i, message in enumerate(retrieved_messages):
        ...
        assert message.content == expected_content, f"Message at index {i} has content '{message.content}', expected '{expected_content}'"

    assert retrieved_messages[-1].content == f"Message {large_number - 1}", f"Last message content is '{retrieved_messages[-1].content}', expected 'Message {large_number - 1}'"

def test_empty_messages(chat_history):
    assert len(chat_history.messages) == 0

def test_json_structure(redis_client, chat_history):
    chat_history.add_message(HumanMessage(content="Test message"))
    ...
    assert json_data["data"]["content"] == "Test message", "Content should match the added message"
    assert json_data["data"]["type"] == "human", "Type should be 'human' for a HumanMessage"

    # Check the type at the root level
    assert json_data["type"] == "human", "Type at root level should be 'human' for a HumanMessage"

    # Check the session_id
    assert json_data["session_id"] == chat_history.session_id, "session_id should match the chat history session_id"

def test_search_non_existent_message(chat_history):
    chat_history.add_message(HumanMessage(content="Hello, how are you?"))
    results = chat_history.search_messages("nonexistent")
    assert len(results) == 0

def test_add_message_to_existing_session(redis_url):
