...
def test_redis_config_default_initialization():
    config = RedisConfig()
...
def test_redis_config_custom_initialization():
    config = RedisConfig(
...
def test_redis_config_with_metadata_schema():
    metadata_schema = [
        {"name": "color", "type": "tag"},
...
def test_redis_config_to_index_schema(embeddings):
...
def test_redis_config_from_existing_index(redis_url):
...
def test_redis_vector_store_with_config(redis_url, embeddings, sample_texts):
...
def test_redis_vector_store_from_texts_with_config(redis_url, embeddings, sample_texts):
    config = RedisConfig(index_name=f"test_index_{uuid.uuid4().hex}")
...
def test_redis_vector_store_from_documents_with_config(redis_url, embeddings):
...
def test_redis_config_with_custom_schema(redis_url, embeddings):
...
def test_redis_config_with_yaml_schema(tmp_path, redis_url, embeddings):
