...
class TestRedisChatMessageHistory:
    ...
    def test_initialization(self, chat_history):
    ...
    def test_search_messages(self, chat_history):
        with patch.object(chat_history.redis_client, 'ft') as mock_ft:
            ...
            expected_query_string = f"(@session_id:{{{chat_history.session_id}}}) (@content:weather)"
            ...
            assert 0 in args  # Note: this is now checking for integer 0, not string '0'
            assert 5 in args  # Note: this is now checking for integer 5, not string '5'
    ...
    def test_len(self, chat_history):
    ...
    def test_ttl(self, chat_history):
        chat_history.add_message(HumanMessage(content="Test message"))
        assert len(chat_history.messages) == 1

        # Manually clear the data to simulate TTL expiration
        chat_history.redis_client._json.data.clear()
        assert len(chat_history.messages) == 0

    def test_ensure_index(self, chat_history):
        with patch.object(chat_history.redis_client, 'ft') as mock_ft:
            # Mock the info method to raise an exception (simulating index doesn't exist)
            mock_ft.return_value.info.side_effect = Exception("Index not found")
            ...
            assert mock_ft.call_args_list == [call(chat_history.index_name), call(chat_history.index_name)]
            ...
            assert schema[0].redis_args() == ['$.session_id', 'AS', 'session_id', 'TAG', 'SEPARATOR', ',']

            assert isinstance(schema[1], TextField)
            assert schema[1].redis_args() == ['$.data.content', 'AS', 'content', 'TEXT', 'WEIGHT', 1.0]

            assert isinstance(schema[2], TagField)
            assert schema[2].redis_args() == ['$.type', 'AS', 'type', 'TAG', 'SEPARATOR', ',']

            assert isinstance(schema[3], NumericField)
            assert schema[3].redis_args() == ['$.timestamp', 'AS', 'timestamp', 'NUMERIC']
            ...
            # Print out the args of the IndexDefinition object
            print(f"IndexDefinition args: {index_definition.args}")
        ...
        # Check that the index is created after calling _ensure_index
        assert chat_history.index_name in chat_history.redis_client.indices

    def test_add_user_message(self, chat_history):
    ...
    def test_add_ai_message(self, chat_history):
    ...
    def test_multiple_sessions(self, mock_redis):
        # First session
        with patch("redis.Redis.from_url", return_value=mock_redis):
            ...
            history1.add_ai_message("Why did the chicken cross the road? To get to the other side!")
        ...
        assert messages1[3].content == "Why did the chicken cross the road? To get to the other side!"
        ...
        assert messages2[3].content == "Why did the chicken cross the road? To get to the other side!"

    def test_add_user_and_ai_messages(self, chat_history):
